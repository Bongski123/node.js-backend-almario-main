...
router.post('/register', async (req, res) =>{
    ...
    try {
        ...
        const {firstName,middleName, lastName,suffix,password,email,role_id} = req.body;
        const hashedPassword = await bcrypt.hash(password, 10);

        const insertUsersQuery = 'INSERT INTO users (firstName,middleName,lastName,suffix,password,email,role_id) VALUES (?,?,?,?,?,?,?)';
        await db.promise().execute(insertUsersQuery,[firstName,middleName, lastName,suffix,hashedPassword,email,role_id]);
    ...
    router.post('/login', async (req, res) => {
        try {
            ...
            const tokenPayload = {
                Id: user.user_id,
                email: user.email,
                role_id: user.role_id // Assuming role_name is a column in your users table
            };
...
router.delete('/user/:id',  (req, res) => {
    ...
    try {
        ...
        db.query('DELETE FROM users WHERE user_id =?', id,(err, result, field)=>{
            if (err){
